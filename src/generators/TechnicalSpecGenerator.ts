...
export class TechnicalSpecGenerator {
    ...
    async analyzeArchitecture(analysisResult: FolderContext): Promise<ArchitectureAnalysis> {
        ...
        return {
            systemOverview: await this.generateSystemOverview(analysisResult, { technicalLevel: 'intermediate' } as TechnicalSpecOptions),
            components,
            dataFlow,
            patterns,
            scalingConsiderations: await this.analyzeScalingConsiderations(analysisResult)
        };
    }
    ...
    private async identifyComponents(
        ...
        for (const [componentName, files] of componentGroups) {
            const component = {
                ...
                description: await this.generateComponentDescription(componentName, files),
    ...
    private extractComponentFromPath(filePath: string): string {
        ...
        for (let i = 0; i < pathParts.length; i++) {
            ...
            if (part.includes('service') || part.includes('controller') ||
                part.includes('model') || part.includes('component') ||
                part.includes('util') || part.includes('lib')) {
                return part;
            }
        }
    ...
    private async generateComponentDescription(name: string, files: any[]): Promise<string> {
        ...
        for (const file of files) {
            if (file.content) {
                ...
                for (const func of functions) {
                    ...
                    if (func.includes('get') || func.includes('fetch') || func.includes('read')) {
    ...
    private async analyzeDataFlow(
        ...
        const apiFiles = analysisResult.files.filter(f =>
            f.path.includes('route') || f.path.includes('controller') || f.path.includes('handler')
        );
    ...
    private extractDataFlowFromAPI(file: any): ArchitectureAnalysis['dataFlow'] {
        ...
        const routeMatches = file.content.match(/\.(get|post|put|delete|patch)\s*\(['"`]([^'"`]+)['"`]/g) || [];
    ...
    private inferDataType(method: string): string {
        switch (method.toLowerCase()) {
            case 'get': return 'Query parameters, Response data';
            case 'post': return 'Request body, Response data';
            case 'put': return 'Request body, Updated data';
            case 'delete': return 'Resource identifier';
            case 'patch': return 'Partial update data';
            default: return 'Data';
        }
    }
    ...
    private async identifyArchitecturalPatterns(
        ...
        if (this.hasPattern(analysisResult, 'mvc')) {
            patterns.push({
                ...
                description: 'Separation of concerns between data, presentation, and control logic',
                implementation: ['Models for data layer', 'Views for presentation', 'Controllers for business logic'],
                benefits: ['Clear separation of concerns', 'Maintainable code structure', 'Testability'],
                tradeoffs: ['Increased complexity for simple applications', 'Potential over-engineering']
            });
        }
        ...
        if (this.hasPattern(analysisResult, 'repository')) {
            patterns.push({
                ...
                implementation: ['Repository interfaces', 'Concrete repository implementations', 'Domain models'],
                benefits: ['Testable data access', 'Flexible data sources', 'Clean architecture'],
                tradeoffs: ['Additional abstraction layer', 'Potential over-abstraction']
            });
        }
    ...
    private hasPattern(analysisResult: FolderContext, pattern: string): boolean {
        ...
        switch (pattern) {
            case 'mvc':
                return paths.some(p => p.includes('model')) &&
    ...
    private async analyzeScalingConsiderations(analysisResult: FolderContext): Promise<string[]> {
        const considerations: string[] = [];

        // Analyze for scaling patterns
        const hasDatabase = analysisResult.files.some(f =>
            f.path.includes('database') || f.path.includes('db') || f.content?.includes('query')
        );
        ...
        const hasCaching = analysisResult.files.some(f =>
            f.content?.includes('cache') || f.content?.includes('redis')
        );
        ...
        const hasAPI = analysisResult.files.some(f =>
    ...
    async generateAPISpecification(analysisResult: FolderContext): Promise<APISpecification> {
        const endpoints = await this.extractAPIEndpoints(analysisResult);
        const schemas = await this.extractAPISchemas(analysisResult);
        const authentication = await this.extractAuthenticationMethods(analysisResult);
    ...
    private async extractAPIEndpoints(
        ...
        const apiFiles = analysisResult.files.filter(f =>
            f.path.includes('route') || f.path.includes('controller') ||
            f.path.includes('handler') || f.path.includes('api')
        );
    ...
    private async extractAPISchemas(
        ...
        const modelFiles = analysisResult.files.filter(f =>
            f.path.includes('model') || f.path.includes('schema') || f.path.includes('type')
        );
    ...
    private async extractAuthenticationMethods(
        ...
        const authFiles = analysisResult.files.filter(f =>
            f.path.includes('auth') || f.path.includes('security') || f.path.includes('middleware')
        );
        ...
        for (const file of authFiles) {
            if (file.content) {
                ...
                if (file.content.includes('basic') || file.content.includes('basicAuth')) {
                    authMethods.push({
                        type: 'basic',
        ...
        return authMethods.length > 0 ? authMethods : [{
            type: 'bearer',
    ...
    async generateDatabaseSchema(analysisResult: FolderContext): Promise<DatabaseSchema> {
    ...
    private async identifyDatabases(
        ...
        const dbFiles = analysisResult.files.filter(f =>
    ...
    private extractColumnsFromModel(
        ...
        while ((match = fieldPattern.exec(content)) !== null) {
            ...
            const column = {
                ...
                constraints: [],
                foreignKey: fieldDef.includes('references') ? 'Referenced table' : undefined
            };
    ...
    private async extractMigrations(
        ...
        const migrationFiles = analysisResult.files.filter(f =>
            f.path.includes('migration') || f.path.includes('migrate')
        );
    ...
    private async extractSeedData(
        ...
        const seedFiles = analysisResult.files.filter(f =>
            f.path.includes('seed') || f.path.includes('fixture')
        );
    ...
    async generateConfigurationDocs(analysisResult: FolderContext): Promise<ConfigurationDocumentation> {
    ...
    private async extractEnvironments(
        ...
        const envFiles = analysisResult.files.filter(f =>
    ...
    private async extractConfigFiles(
        ...
        const configs = analysisResult.files.filter(f =>
        ...
        try {
            ...
            for (const [key, value] of Object.entries(config)) {
                sections.push({
                    ...
                    options: typeof value === 'object' ? Object.keys(value as object) : [String(value)]
                });
            }
        } catch {
    ...
    private async extractSecrets(
        ...
        const hasSecretManager = analysisResult.files.some(f =>
            f.content?.includes('secrets') || f.content?.includes('vault') ||
    ...
    async generateDeploymentDocs(analysisResult: FolderContext): Promise<DeploymentDocumentation> {
        const environments = await this.extractDeploymentEnvironments(analysisResult);
        ...
        const troubleshooting = await this.generateDeploymentTroubleshooting(analysisResult);
    ...
    private async extractDeploymentEnvironments(
        ...
        const environments: DeploymentDocumentation['environments'] = [
            ...
            },
            {
                ...
                infrastructure: ['Load balancer', 'Multiple app servers', 'Production database'],
        ...
        return [
            {
                ...
                diagnosis: ['Check application logs', 'Verify configuration', 'Check dependencies'],
                resolution: ['Fix configuration errors', 'Install missing dependencies', 'Update environment variables']
            },
            {
                issue: 'Database Connection Errors',
                symptoms: ['Connection timeouts', 'Authentication failures'],
                diagnosis: ['Check database connectivity', 'Verify credentials', 'Check network configuration'],
                resolution: ['Update connection strings', 'Fix network rules', 'Restart database service']
            },
            {
                issue: 'Performance Degradation',
                symptoms: ['Slow response times', 'High resource usage'],
                diagnosis: ['Check system metrics', 'Analyze database queries', 'Review application logs'],
                resolution: ['Scale resources', 'Optimize queries', 'Implement caching']
            }
        ];
    }

    async generateTestingStrategy(analysisResult: FolderContext): Promise<TestingStrategy> {
    ...
    private async analyzeTestAutomation(
        ...
        const automation: TestingStrategy['automation'] = [
            ...
            },
            {
                ...
                pipeline: ['Run all tests', 'Integration tests', 'E2E tests', 'Deploy to staging'],
    ...
    async generateIntegrationMapping(analysisResult: FolderContext): Promise<IntegrationMapping> {
        const externalServices = await this.identifyExternalServices(analysisResult);
        const internalServices = await this.identifyInternalServices(analysisResult);
    ...
    private async identifyInternalServices(
        ...
        const serviceFiles = analysisResult.files.filter(f =>
    ...
    private extractServiceName(path: string): string {
        const fileName = path.split('/').pop() || '';
        return fileName.replace(/\.(js|ts|service|api)$/gi, '').replace(/[-_]/g, ' ');
    }
    ...
    private extractFunctions(content: string): string[] {
        ...
        const arrowFunctions = content.match(/(\w+)\s*[=:]\s*(?:async\s+)?\(/g) || [];
...
**Technical Level:** ${options.technicalLevel}`;
    ...
    private inferPrimaryLanguage(analysisResult: FolderContext): string {
        const extensions = analysisResult.files.map(f => f.path.split('.').pop()?.toLowerCase());
        ...
        const primaryExt = Object.keys(langCount).reduce((a, b) =>
            langCount[a] > langCount[b] ? a : b, 'js'
        );
    ...
    // Formatting methods
    private formatArchitectureAnalysis(architecture: ArchitectureAnalysis): string {
        const componentsSection = architecture.components.map(comp =>
            `### ${comp.name} (${comp.type})
...
**Files:** ${comp.files.join(', ')}`
        ...
        const patternsSection = architecture.patterns.map(pattern =>
            `### ${pattern.name}
...
${architecture.scalingConsiderations.map(sc => `- ${sc}`).join('\n')}`;
    ...
    private formatAPISpecification(apiSpec: APISpecification): string {
        const endpointsSection = apiSpec.endpoints.map(endpoint =>
...
**Authentication:** ${endpoint.authentication.join(', ')}`
        ...
        const schemasSection = apiSpec.schemas.map(schema =>
            `### ${schema.name}
**Type:** ${schema.type}
...
${apiSpec.authentication.map(auth => `- **${auth.type}**: ${auth.description}`).join('\n')}`;
    ...
    private formatDatabaseSchema(dbSchema: DatabaseSchema): string {
        const databasesSection = dbSchema.databases.map(db => {
            const tablesSection = db.tables.map(table =>
                `#### ${table.name}
...
${dbSchema.seedData.map(s => `- **${s.table}**: ${s.description}`).join('\n')}`;
    ...
    private formatConfigurationDocs(config: ConfigurationDocumentation): string {
        const environmentsSection = config.environments.map(env =>
            `### ${env.name}
...
${config.secrets.map(s => `- **${s.name}**: ${s.description} (Storage: ${s.storage})`).join('\n')}`;
    ...
    private formatDeploymentDocs(deployment: DeploymentDocumentation): string {
        const environmentsSection = deployment.environments.map(env =>
            `### ${env.name}
**Description:** ${env.description}
...
**Scaling:** ${env.scaling}`
        ...
        const stepsSection = deployment.deploymentSteps.map(step =>
            `### ${step.phase}
...
${deployment.troubleshooting.map(t => `- **${t.issue}**: ${t.symptoms.join(', ')} → ${t.resolution.join(', ')}`).join('\n')}`;
    ...
    private formatTestingStrategy(testing: TestingStrategy): string {
        const levelsSection = testing.levels.map(level =>
            `### ${level.name}
...
${testing.automation.map(ta => `- **${ta.trigger}**: ${ta.description}`).join('\n')}`;
    ...
    private formatIntegrationMapping(integration: IntegrationMapping): string {
        const externalSection = integration.externalServices.map(service =>
            `### ${service.name}
**Type:** ${service.type}
...
**Data Format:** ${service.dataFormat}`
        ...
        const internalSection = integration.internalServices.map(service =>
            `### ${service.name}
**Description:** ${service.description}
