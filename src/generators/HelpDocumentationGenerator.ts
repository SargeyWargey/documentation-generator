...
export class HelpDocumentationGenerator {
  ...
  async generateHelpDocumentation(
    ...
    if (options.includeGettingStarted) {
      const gettingStarted =
        await this.generateGettingStartedSection(analysisResult);
    ...
    if (options.includeTroubleshooting) {
      const troubleshooting =
        await this.generateTroubleshootingSection(analysisResult);
    ...
    const templateVariables = {
      ...
      projectDescription:
        analysisResult.summary || 'A comprehensive software project',
      installationSteps: await this.generateInstallationSteps(analysisResult),
      mainFeatures: features,
      apiEndpoints: options.includeAPIReference
        ? await this.detectAndParseAPIDocumentation(analysisResult)
        : [],
      commonIssues: options.includeTroubleshooting
        ? await this.generateTroubleshootingSection(analysisResult)
        : [],
      faq: options.includeFAQ
        ? await this.generateFAQSection(analysisResult)
        : [],
      contactInfo:
        'Please check the project repository for support information',
      version: '1.0.0',
      analysis: {
  ...
  async detectAndParseAPIDocumentation(
    analysisResult: FolderContext
  ...
  private isAPIFile(file: any): boolean {
    ...
    return (
      apiPatterns.some((pattern) => pattern.test(file.path)) ||
      file.exports?.some(
        (exp: string) => exp.includes('API') || exp.includes('Controller')
      )
    );
  }
  ...
  private assessFeatureComplexity(files: any[]): 'low' | 'medium' | 'high' {
    ...
    if (totalLines > 5000 || fileCount > 10) {
      return 'high';
    }
    if (totalLines > 1000 || fileCount > 5) {
      return 'medium';
    }
    return 'low';
  }

  async generateGettingStartedSection(
    analysisResult: FolderContext
  ): Promise<GettingStartedSection> {
    const packageJson = analysisResult.files.find((f) =>
      f.path.endsWith('package.json')
    );
    const readme = analysisResult.files.find((f) =>
      f.path.toLowerCase().includes('readme')
    );
  ...
  private async generateBasicUsage(
    ...
    const mainFile = analysisResult.files.find(
      (f) =>
    ...
    if (mainFile) {
      usage.push(
        `Import the main module: import { } from '${analysisResult.name}'`
      );
    }
  ...
  private async generateFirstExample(
    analysisResult: FolderContext
  ): Promise<string> {
    // Find the simplest example or create a basic one
    const exampleFiles = analysisResult.files.filter(
      (f) =>
  ...
  async generateTroubleshootingSection(
    analysisResult: FolderContext
  ...
  private async extractFAQFromCodeComments(
    analysisResult: FolderContext
  ): Promise<FAQEntry[]> {
  ...
  // Formatting methods
  private formatGettingStartedSection(
...
${gettingStarted.nextSteps.map((step) => `- ${step}`).join('\n')}`;
  ...
  private formatAPIDocumentation(apiDocs: APIDocumentation[]): string {
    ...
    const sections = apiDocs.map(
      (doc) => `### ${doc.className}.${doc.methodName}
...
${doc.examples.length > 0 ? `**Examples:**\n${doc.examples.map((ex) => `\`\`\`javascript\n${ex}\n\`\`\``).join('\n')}` : ''}`
  ...
  private formatTroubleshootingSection(
    ...
    const entries = troubleshooting.map(
      (entry) => `### ${entry.problem}
...
**Tags:** ${entry.tags.join(', ')}`
  ...
  private async extractMainFeatures(
    analysisResult: FolderContext
  ): Promise<
    Array<{
      name: string;
  ...
  > {
    ...
    const featureFiles = analysisResult.files.filter((file) => {
      ...
      return (
        codeStructure &&
        codeStructure.exports.length > 0 &&
    ...
    for (const file of featureFiles.slice(0, 5)) {
      ...
      const feature = {
        ...
        usage:
          exports.length > 0
            ? `import { ${exports[0].name} } from '${file.path}'`
            : undefined,
        examples:
          exports
            .slice(0, 2)
