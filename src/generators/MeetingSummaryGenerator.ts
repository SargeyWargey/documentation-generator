...
export class MeetingSummaryGenerator {
    ...
    async generateMeetingSummary(
        ...
        const participantSummary = this.generateParticipantSummary(meetingContext.participants);
        sections.push(participantSummary);

        if (options.includeMeetingNotes) {
            const notes = await this.extractMeetingNotes(discussionContent, meetingContext);
            sections.push(this.formatMeetingNotes(notes));
        }

        if (options.includeActionItems) {
            const actionItems = await this.extractActionItems(discussionContent, meetingContext);
            sections.push(this.formatActionItems(actionItems));
        }

        if (options.includeDecisionTracking) {
            const decisions = await this.extractDecisions(discussionContent, meetingContext);
            sections.push(this.formatDecisions(decisions));
        }

        if (options.includeFollowUpTasks) {
            const followUpTasks = await this.generateFollowUpTasks(discussionContent, meetingContext);
        ...
        if (options.includeSeriesTracking && meetingContext.meetingSeriesId) {
            const seriesAnalysis = await this.analyzeMeetingSeries(meetingContext.meetingSeriesId);
    ...
    private cleanActionDescription(description: string): string {
        return description
            ...
            .replace(/[^.!?]*$/, match => match.endsWith('.') ? match : match + '.');
    }
    ...
    private inferPriority(description: string, context: string): 'high' | 'medium' | 'low' {
        const highPriorityKeywords = ['urgent', 'critical', 'asap', 'immediately', 'blocker', 'high priority'];
        const mediumPriorityKeywords = ['important', 'needed', 'required', 'should'];
    ...
    private categorizeActionItem(description: string): string {
        const categories = {
            ...
            'communication': ['email', 'call', 'inform', 'notify', 'share', 'communicate'],
    ...
    private extractDependencies(description: string, _existingItems: ActionItem[]): string[] {
    ...
    private requiresFollowUp(description: string): boolean {
        const followUpKeywords = ['follow up', 'check back', 'monitor', 'track', 'review progress'];
        const lowerDesc = description.toLowerCase();
    ...
    private enhanceActionItems(
        ...
        return actionItems.map(item => ({
            ...item,
            relatedDecisions: this.findRelatedDecisions(item.description, discussionContent),
        }));
    }

    private findRelatedDecisions(actionDescription: string, discussionContent: string): string[] {
    ...
    private isRelatedToAction(decision: string, action: string): boolean {
        ...
        const overlap = decisionWords.filter(word =>
    ...
    async extractDecisions(discussionContent: string, context: MeetingContext): Promise<Decision[]> {
            ...
            while ((match = pattern.exec(discussionContent)) !== null) {
                ...
                if (title && title.length > 10) {
                    const decision: Decision = {
                        ...
                        description: this.extractDecisionDescription(title, discussionContent),
    ...
    private extractAlternatives(title: string, content: string): Array<{ option: string; pros: string[]; cons: string[]; rejected: boolean; reason?: string; }> {
    ...
    private assessDecisionImpact(title: string, content: string): 'high' | 'medium' | 'low' {
        const highImpactKeywords = ['critical', 'major', 'significant', 'substantial', 'strategic'];
    ...
    private extractStakeholders(_title: string, participants: Participant[]): string[] {
    ...
    private categorizeFollowUpTask(description: string): 'research' | 'implementation' | 'communication' | 'review' | 'decision' {
        const categories = {
            'research': ['research', 'investigate', 'analyze', 'study', 'explore', 'look into'],
            'implementation': ['implement', 'build', 'create', 'develop', 'setup'],
            'communication': ['contact', 'email', 'call', 'inform', 'notify', 'coordinate'],
        ...
        for (const [category, keywords] of Object.entries(categories)) {
            if (keywords.some(keyword => lowerDesc.includes(keyword))) {
                return category as 'research' | 'implementation' | 'communication' | 'review' | 'decision';
            }
        }
    ...
    private extractTaskAssignee(text: string, participants: Participant[]): string {
    ...
    private extractTaskContext(description: string, fullContent: string): string {
        const descIndex = fullContent.toLowerCase().indexOf(description.toLowerCase());
        if (descIndex !== -1) {
            const start = Math.max(0, descIndex - 100);
            const end = Math.min(fullContent.length, descIndex + description.length + 100);
    ...
    private inferDeliverable(description: string): string {
        ...
        if (description.toLowerCase().includes('contact') || description.toLowerCase().includes('email')) {
    ...
    private identifyRequiredResources(description: string): string[] {
        ...
        if (description.toLowerCase().includes('research')) {
            resources.push('Access to documentation', 'Research tools', 'Subject matter experts');
        }
        if (description.toLowerCase().includes('implement')) {
            resources.push('Development environment', 'Technical specifications', 'Testing resources');
        }
        if (description.toLowerCase().includes('coordinate') || description.toLowerCase().includes('contact')) {
            resources.push('Contact information', 'Communication channels', 'Calendar access');
        }
    ...
    async extractMeetingNotes(
        ...
        const importantQuotes = await this.extractImportantQuotes(discussionContent, context.participants);
        const concerns = await this.extractConcerns(discussionContent, context.participants);
        const achievements = await this.extractAchievements(discussionContent, context.participants);
        const challenges = await this.extractChallenges(discussionContent, context.participants);
    ...
    private inferSpeaker(index: number, content: string, participants: Participant[]): string {
        // Look for speaker indicators before the quote
        const precedingText = content.slice(Math.max(0, index - 100), index);

        for (const participant of participants) {
            if (precedingText.toLowerCase().includes(participant.name.toLowerCase())) {
    ...
    private assessQuoteSignificance(quote: string): string {
        ...
        for (const [significance, keywords] of Object.entries(significanceKeywords)) {
    ...
    private async extractConcerns(content: string, participants: Participant[]): Promise<any[]> {
        const concerns: any[] = [];
    ...
    private assessConcernImpact(concern: string): string {
        ...
        if (mediumImpactKeywords.some(keyword => lowerConcern.includes(keyword))) {
    ...
    private async extractAchievements(content: string, participants: Participant[]): Promise<any[]> {
        ...
        for (const pattern of achievementPatterns) {
            ...
            while ((match = pattern.exec(content)) !== null) {
                ...
                if (achievement && achievement.length > 10) {
                    achievements.push({
                        achievement,
                        contributors: this.extractContributors(achievement, content, participants),
    ...
    private extractContributors(achievement: string, content: string, participants: Participant[]): string[] {
        const contributors: string[] = [];

        // Look for names mentioned near the achievement
        const achievementIndex = content.toLowerCase().indexOf(achievement.toLowerCase());
        if (achievementIndex !== -1) {
            const contextStart = Math.max(0, achievementIndex - 100);
            const contextEnd = Math.min(content.length, achievementIndex + achievement.length + 100);
    ...
    private async extractChallenges(content: string, participants: Participant[]): Promise<any[]> {
            ...
            while ((match = pattern.exec(content)) !== null) {
                ...
                if (challenge && challenge.length > 10) {
                    challenges.push({
                        ...
                        proposedSolutions: this.extractProposedSolutions(challenge, content),
    ...
    private extractChallengeOwner(challenge: string, content: string, participants: Participant[]): string {
    ...
    async analyzeParticipantContributions(
        ...
        const insights = {
            participationLevels: this.analyzeParticipationLevels(discussionContent, participants),
            contributionTypes: this.analyzeContributionTypes(discussionContent, participants),
            expertiseShown: this.analyzeExpertiseShown(discussionContent, participants),
            engagementMetrics: this.calculateEngagementMetrics(discussionContent, participants)
        };

        return insights;
    }

    private analyzeParticipationLevels(content: string, participants: Participant[]): { participant: string; level: string; }[] {
    ...
    private analyzeContributionTypes(content: string, participants: Participant[]): { participant: string; types: string[]; }[] {
        const contributionTypes = ['questions', 'suggestions', 'concerns', 'decisions', 'information'];

        return participants.map(participant => {
            const contributions: { [key: string]: number } = {};

            for (const type of contributionTypes) {
                contributions[type] = this.countContributionType(participant.name, type, content);
            }

            return {
                participant: participant.name,
                types: Object.keys(contributions).filter(key => contributions[key] > 0)
            };
        });
    }

    private countContributionType(name: string, type: string, content: string): number {
    ...
    private analyzeExpertiseShown(content: string, participants: Participant[]): { participant: string; expertise: string[]; }[] {
        return participants.map(participant => {
    ...
    private identifyExpertiseAreas(name: string, content: string): string[] {
        const expertiseKeywords = {
            'technical': ['code', 'architecture', 'system', 'implementation', 'technical'],
    ...
    private calculateEngagementMetrics(content: string, participants: Participant[]): { speakingTime: number; contributions: number; questions: number; } {
        const totalMentions = participants.reduce((sum, p) =>
            sum + this.countMentions(p.name, content), 0
        );

        const averageParticipation = totalMentions / participants.length;

        const highParticipants = participants.filter(p =>
            this.countMentions(p.name, content) > averageParticipation
        ).length;
...
**Attendance Rate:** ${(participants.filter(p => p.attendance === 'present').length / participants.length * 100).toFixed(1)}%`;
    ...
    private formatMeetingNotes(notes: MeetingNotes): string {
        const topicsSection = notes.keyTopics.map(topic =>
            `### ${topic.topic}
${topic.discussion}
...
**Next Steps:** ${topic.nextSteps.join(', ') || 'None specified'}`
        ...
        const quotesSection = notes.importantQuotes.map(quote =>
            `> "${quote.quote}" - **${quote.speaker}**\n*${quote.significance}*`
        ).join('\n\n');

        const concernsSection = notes.concerns.map(concern =>
...
${items}`;
    ...
    private formatParticipantInsights(insights: any): string {
        const participationSection = insights.participationLevels.map((p: any) =>
...
${engagementSection}`;
    ...
    private formatSeriesAnalysis(series: MeetingSeries): string {
        const trendsSection = series.trends.map(trend =>
