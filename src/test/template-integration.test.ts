...
`;
    ...
    await fs.writeFile(
      path.join(testTemplatesPath, 'help-template.md'),
      helpTemplate
    );
    await fs.writeFile(
      path.join(testTemplatesPath, 'prd-template.md'),
      prdTemplate
    );
    await fs.writeFile(
      path.join(testTemplatesPath, 'technical-template.md'),
      technicalTemplate
    );
    await fs.writeFile(
      path.join(testTemplatesPath, 'meeting-template.md'),
      meetingTemplate
    );
  }
  ...
  describe('Template Loading and Validation', () => {
    ...
    it('should extract template variables correctly', async () => {
      ...
      await testTemplateManager.initialize();
      const variables =
        testTemplateManager.getTemplateVariables('help-template');
      ...
      assert.strictEqual(
        projectNameVar.type,
        'string',
        'projectName should be string type'
      );
      assert.strictEqual(
        projectNameVar.required,
        true,
        'projectName should be required'
      );
    });
    ...
    it('should handle missing variables gracefully', async () => {
      ...
      try {
        await testTemplateManager.processTemplate(
          'help-template',
          incompleteVariables
        );
        ...
        assert.ok(
          ...
          'Should indicate missing variables'
        );
      }
    });
  });
  ...
  describe('Variable Substitution', () => {
    ...
    it('should substitute simple variables', async () => {
      ...
      const result = await testTemplateManager.processTemplate(
        'help-template',
        variables
      );

      assert.ok(
        result.includes('Amazing Project'),
        'Should substitute projectName'
      );
      assert.ok(
        result.includes('An amazing test project'),
        'Should substitute projectDescription'
      );
    });
    ...
    it('should process array iterations', async () => {
      ...
      assert.ok(
        result.includes('First feature'),
        'Should include first feature description'
      );
    });
    ...
    it('should process simple array values', async () => {
      const variables = {
        projectName: 'Test Project',
        projectDescription: 'Test description',
        installationSteps: ['npm install', 'npm start', 'Open browser'],
      };
      ...
      assert.ok(
        result.includes('Open browser'),
        'Should include browser instruction'
      );
    });
    ...
    it('should handle analysis data patterns', async () => {
      ...
      const result = await testTemplateManager.processTemplateContent(
        content,
        variables
      );

      assert.ok(
        result.includes('Test Project'),
        'Should substitute main projectName'
      );
      assert.ok(
        result.includes('Analysis Project'),
        'Should substitute analysis.projectName'
      );
      assert.ok(
        result.includes('Files: 10'),
        'Should substitute analysis.fileCount'
      );
    });
    ...
    it('should add common variables automatically', async () => {
      ...
      assert.ok(
        result.includes(currentYear),
        'Should include current year from currentDate'
      );
    });
  });
  ...
  describe('Content Generation with Real Data', () => {
    ...
    beforeEach(async () => {
      ...
      technicalGenerator = new TechnicalSpecGenerator(
        testTemplateManager,
        claudeIntegrator
      );
    ...
    it('should generate help documentation with template', async () => {
      ...
      const result = await helpGenerator.generateHelpDocumentation(
        folderContext,
        options
      );
      ...
      assert.ok(
        result.includes('User Guide'),
        'Should include template header'
      );
      assert.ok(
    ...
    it('should generate PRD with template', async () => {
      ...
      assert.ok(result.includes('TestProject'), 'Should include project name');
      assert.ok(
        result.includes('Product Requirements Document'),
        'Should include template header'
      );
      assert.ok(
        result.includes('Product Vision'),
        'Should include product vision section'
      );
      assert.ok(result.length > 100, 'Should generate substantial content');
    });
    ...
    it('should generate technical specification with template', async () => {
      ...
      assert.ok(
        result.includes('Technical Specification'),
        'Should include template header'
      );
      assert.ok(
        result.includes('Architecture Overview'),
    ...
    it('should generate meeting summary with template', async () => {
      ...
      const discussionContent =
        'We discussed the sprint goals and assigned tasks to team members.';
      ...
      assert.ok(
        result.includes('Sprint Planning Meeting'),
        'Should include meeting title'
      );
  ...
  describe('Error Handling', () => {
    ...
    it('should validate variable types', async () => {
      ...
      const archTypeVar = template.metadata.variables?.find(
        (v) => v.name === 'architectureType'
      );
      ...
      assert.ok(
        archTypeVar.options && archTypeVar.options.length > 0,
        'Select variable should have options'
      );
    });

    it('should handle unprocessed template syntax', async () => {
      const contentWithUnprocessed =
        '{{unprocessedVariable}} and {{#nonExistentArray}}{{/nonExistentArray}}';
      const result = await testTemplateManager.processTemplateContent(
        contentWithUnprocessed,
        {}
      );

      // Should clean up unprocessed syntax
      assert.ok(
        !result.includes('{{unprocessedVariable}}'),
        'Should remove unprocessed variables'
      );
      assert.ok(
        !result.includes('{{#nonExistentArray}}'),
        'Should remove unprocessed array syntax'
      );
    });
  });
  ...
  describe('All Template Types Generation', () => {
    it('should generate content for all template types', async () => {
      ...
      for (const templateName of templates) {
        ...
        switch (templateName) {
          ...
          case 'prd-template':
            variables = {
              ...
              businessGoals: [
                { goal: 'Increase satisfaction', rationale: 'User feedback' },
              ],
            };
            break;
        ...
        assert.ok(
          result.length > 50,
          `Template ${templateName} should generate content`
        );
        assert.ok(
          !result.includes('{{'),
          `Template ${templateName} should not have unprocessed variables`
        );
      }
    });
  });
