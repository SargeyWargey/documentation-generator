...
suite('Documentation Generation Integration Tests', () => {
  ...
  test('Should generate help documentation for test folder', async function() {
    ...
    try {
      ...
      const exists = await fs.access(testFolderPath).then(() => true).catch(() => false);
      assert.strictEqual(exists, true, 'Test folder should exist');

      // Execute the generateDocs command
      const result = await vscode.commands.executeCommand('documentation-generator.generateDocs');

      // Check if documentation file was created
      const helpDocPath = path.join(testFolderPath, 'help-documentation.md');
      const docExists = await fs.access(helpDocPath).then(() => true).catch(() => false);

      if (docExists) {
        const content = await fs.readFile(helpDocPath, 'utf-8');
        assert.ok(content.length > 0, 'Generated documentation should not be empty');
        console.log('✅ Help documentation generated successfully');

        // Clean up
        await fs.unlink(helpDocPath).catch(() => {});
      } else {
        console.log('⚠️  Command executed but no file was created (manual selection may be required)');
      }
  ...
  test('Should handle invalid folder paths gracefully', async function() {
    ...
    try {
      ...
      assert.ok(error.message.length > 0, 'Error message should be informative');
    }
  });

  test('Should show extension as active', async function() {
    const extension = vscode.extensions.getExtension('your-publisher.documentation-generator');

    if (extension) {
      assert.strictEqual(extension.isActive, true, 'Extension should be active');
      console.log('✅ Extension is active');
    } else {
      console.log('⚠️  Extension not found (may be expected in test environment)');
    }
  });
